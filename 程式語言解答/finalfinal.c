...
int main(){
    ...
    int num1 = 0;
    int num2 = 0;//num1是幾個資料num2是幾個指令
    scanf("%d %d\n",&num1,&num2);
    record* database = malloc(num1 * sizeof(record));
    rightrecord* undatabase = malloc(num1 * sizeof(rightrecord));
    //改成0了
    ...
    for (int i=0; i < num1; i++) {
        scanf("%d %s %d %s %s %d\n", &id, name, &studentid,classroom,email,&phone);
        database[i].id=id;
    ...
    index = 0;//從0開始找
    while(index>=0){
        index=getNextToken(command[i],index,buffer);
        int order=-1;
        int array[7]={0};//出現順序
        if(strcmp(buffer,"SELECT")==0){
            order=selectdatabase(command[i],index,buffer,database,num1,array);
             //印出
        if(order==0){
            ...
            deletedatabase(command[i],index,buffer,database,num1);
...
int finddatabase(char *command,int index,char *buffer,record *database,int num1,int databasecorrect[],int orandnone){
    ...
    if(strlen(buffer)==2){
           if(strcmp(buffer,">=")==0){
            ...
            if(whichsubject==1){
                int number;
                number = strtol(buffer,NULL,0);//把字串變成數字
                for(int i=0;i<num1;i++){
...
void finaldatabase(char *command,int index,char *buffer,record *database,int num1,int databasecorrect[],int howmanycorrect,int orand,int deleselenone){
    ...
    if(deleselenone==1){
        if(orand==0){
            ...
            memset(databasecorrect, 0, sizeof(int) * 301);
            ...
            for(int i=0;i<number;i++){
                    ...
                    for(int v=1;v<7;v++){
                        for(int g=1;g<7;g++){
                            if(array[g]==v){
                                if(g==1){
                                    printf("%d ",undatabase[i].id);
                                    break;
                                }
                                else if(g==2){
                                    printf("%d ",undatabase[i].studentid);
                                    break;
                                }
                                else if(g==3){
    ...
    if(subject==1){
        ...
        for(int i=0;i<count2;i++){
            ...
            if(check2!=-1){
                ...
                undatabase[i].phone=database[check2].phone;
